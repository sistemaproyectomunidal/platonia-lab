...
export async function labServiceExample() {
  ...
  if (aiResponse.data?.text) {
    console.log('\nAI Response:', aiResponse.data.text.substring(0, 100) + '...');
  }
...
export async function socraticServiceExample() {
  ...
  const randomQuestions = await socraticService.getRandomQuestions(3, 'L1_miedo');
...
export async function fileServiceExample() {
  ...
  if (uploads.data) {
    ...
    uploads.data.slice(0, 3).forEach(upload => {
      console.log(`  - ${upload.filename} (${(upload.size_bytes / 1024).toFixed(2)} KB)`);
